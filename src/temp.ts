...
function isCidrAllowed(serviceCidr: string, allowedCidr: string): boolean {
  ...
  return (
    (allowedIpInt & mask) === (serviceIpInt & mask) &&
    allowedMaskInt <= serviceMaskInt
  );
}

function isCidrInAllowedCidrs(
  allowedCidrs: string[],
  serviceCidr: string
): boolean {
  return allowedCidrs.some((allowedCidr) =>
    isCidrAllowed(serviceCidr, allowedCidr)
  );
}
